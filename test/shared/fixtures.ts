...
import PlearnFactory from '../../artifacts/contracts/test/Factory.sol/Factory.json'
...
export async function factoryFixture([wallet]: Wallet[], _: Web3Provider): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, PlearnFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture([wallet]: Wallet[], provider: Web3Provider): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(PlearnPair.abi), provider).connect(wallet)
